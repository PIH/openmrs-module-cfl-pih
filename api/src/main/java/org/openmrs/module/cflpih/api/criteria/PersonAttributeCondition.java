...
public class PersonAttributeCondition extends BaseJoinHibernateCondition {
  ...
  public PersonAttributeCondition(
      PersonAttributeType attributeType, String operator, Object value) {
  ...
  public Void applyCondition(Criteria hibernateCriteria) {
    final DetachedCriteria conditionQuery =
        DetachedCriteria.forClass(PersonAttribute.class, getAlias());
    conditionQuery.setProjection(Property.forName(PERSON_PROP));

    final Criterion joinOnClause =
        and(
            new EntityFieldExpression(
                getAlias() + VALUE_FIELD_SUFFIX, getOperator(), valueAsString),
